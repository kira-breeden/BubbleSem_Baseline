...
// Function to get target word index from the 0-indexed position in the CSV
// The position is the nth WORD (not token), so we need to convert to token index
function getTargetWordTokenIndex(tokens, targetWordPosition) {
    if (targetWordPosition === null || targetWordPosition === undefined || targetWordPosition === '') {
        console.error('No target_word_position provided');
        return -1;
    }
    ...
    if (isNaN(posNum) || posNum < 0) {
        console.error(`Invalid target_word_position: ${targetWordPosition}`);
        return -1;
    }
    
    // Count words (skip punctuation tokens) to find the token index of the nth word
    let wordCount = 0;
    ...
    console.error(`Could not find word at position ${posNum} (only ${wordCount} words found)`);
    return -1;
}
...
function createWordRevealTrial(trialIndex) {
    ...
    // Get target word position (0-indexed) from CSV and convert to token index
    const targetWord = trial.target_word;
    const targetWordPosition = trial.target_word_position;
    const targetIndex = getTargetWordTokenIndex(realTokens, targetWordPosition);
    ...
    console.log(`Trial ${trialNumber}:`, {
        ...
        targetTokenInReal: realTokens[targetIndex],
        targetTokenInJabber: jabberTokens[targetIndex]
    });
    ...
    return {
        ...
        stimulus: function() {
            ...
            trialSequenceData = {
                ...
                target_word_position: targetWordPosition,  // 0-indexed position from CSV
                target_word_index: targetIndex,  // Token index (accounting for punctuation)
