...
const jsPsych = initJsPsych({
    on_data_update: function(data) {
        // Only keep trial data, not instruction/feedback screens
        if (data.trial_type === 'consolidated-trial') {
            // This is the data we want to keep
        } else if (['html-keyboard-response', 'html-button-response', 'survey-text', 'pipe'].includes(data.trial_type)) {
            // Remove unwanted trials from the dataset
            const allData = jsPsych.data.get();
            const filteredData = allData.filter({trial_type: 'consolidated-trial'});
            jsPsych.data.reset();
...
function createWordRevealTrial(trialIndex) {
    ...
    return {
        ...
        data: {
            save_trial: false // Don't save this trial
        }
    };
}
...
function createConfidenceRatingTrial(trialIndex) {
    return {
        ...
        data: {
            save_trial: false // Don't save this trial separately
        }
    };
}
...
function createFeedbackTrial(trialIndex) {
    ...
    return {
        ...
        trial_duration: null,
        data: {
            save_trial: false // Don't save feedback trials
        }
    };
}
